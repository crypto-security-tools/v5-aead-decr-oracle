...
class L_computer final {
   public:
      ...
      explicit L_computer(std::span<const uint8_t> encrypted_zero_block) :
            m_BS(encrypted_zero_block.size()), m_max_blocks(10000) { // arbitrary high value for m_max_blocks
      ...
      void init(const std::vector<uint8_t>& offset) { m_offset = offset; }

      bool initialized() const { return m_offset.empty() == false; }

      const std::vector<uint8_t>& star() const { return m_L_star; }

      const std::vector<uint8_t>& dollar() const { return m_L_dollar; }

      const std::vector<uint8_t>& offset() const { return m_offset; }
